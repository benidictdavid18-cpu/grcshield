"""Capture the four README screenshots from a running instance.

Drives the installed Microsoft Edge through Playwright, so nothing is downloaded. Signs
in as the read-only auditor, because the role chip in the masthead is part of what the
...
def sign_in(page, base: str) -> None:
    ...
    page.fill("#username", AUTOR_USER := AUDITOR[0])
    page.fill("#password", AUDITOR[1])
